...
const hooksContent = `
  const { data: allowance, refetch: refetchAllowance } = useReadContract({
    ...
    query: { enabled: isConnected && sellToken.symbol !== 'DOGE' }
  });
...
const buttonLogicNew = `
  ...
  if (!isConnected) {
    ...
    buttonText = "INSUFFICIENT BALANCE";
    isDisabled = true;
  } else if (sellToken.symbol !== 'DOGE' && allowance !== undefined && allowance < parsedSellWei) {
    needsApproval = true;
...
const confirmNew = `onClick={async () => {
                      ...
                      try {
                        ...
                        const isSellingNative = sellToken.symbol === 'DOGE';
