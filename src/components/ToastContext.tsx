  ...
  return (
    <ToastContext.Provider value={{ addToast }}>
      ...
      <div className="fixed top-20 right-4 z-[200] flex flex-col gap-2 w-80">
        {toasts.map(toast => (
          <div key={toast.id} className={`p-4 border-l-4 shadow-lg flex gap-3 items-start ${
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-white/70 mt-0.5 break-words">{toast.message}</div>
              {toast.link && (
                <a href={toast.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-primary mt-1 hover:text-white">
